...
const isMarkdownTable = (input: string): boolean => {
  ...
  // Check for headings format (lines starting with #)
  if (firstLine.startsWith('#')) {
    // Count how many lines start with #
    const headingLines = lines.filter(line => line.trim().startsWith('#') && line.trim().substring(1).trim().length > 0);
    // If we have at least one heading line, it's markdown headings format
    return headingLines.length > 0;
  }
